...
hidden_dim = 16 #隐藏层维度
epochs = 1 #训练轮次
device = torch.device('cuda:0') if torch.cuda.is_available() else torch.device('cpu') #设备选择
print(device)

#读取数据集
df = pd.read_csv('D:\\Machinelearning cd\\brsNCF\\train_dataset.csv')
...
plt.show()
#测试数据处理
df = pd.read_csv('D:\\Machinelearning cd\\brsNCF\\\\test_dataset.csv')
...
#文件写入
f = open('D:\\Machinelearning cd\\brsNCF\\submission.csv', 'w', encoding='utf-8')
